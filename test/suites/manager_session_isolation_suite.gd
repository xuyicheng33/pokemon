...
func test_manager_session_isolation_interleaved_turns() -> void:
	_assert_legacy_result(_test_manager_session_isolation_interleaved_turns(_harness))

func test_session_seed_and_replay_hash_isolation() -> void:
	_assert_legacy_result(_test_session_seed_and_replay_hash_isolation(_harness))

func test_replay_isolation_no_session_side_effect() -> void:
	_assert_legacy_result(_test_replay_isolation_no_session_side_effect(_harness))
func _test_manager_session_isolation_interleaved_turns(harness) -> Dictionary:
	...
	var init_a = manager.create_session({"battle_seed": 401, "content_snapshot_paths": snapshot_paths, "battle_setup": harness.build_sample_setup(sample_factory)})
	var init_b = manager.create_session({"battle_seed": 402, "content_snapshot_paths": snapshot_paths, "battle_setup": harness.build_sample_setup(sample_factory)})
	...
	if not bool(init_a_unwrap.get("ok", false)):
		return harness.fail_result(str(init_a_unwrap.get("error", "manager create_session A failed")))
	if not bool(init_b_unwrap.get("ok", false)):
		return harness.fail_result(str(init_b_unwrap.get("error", "manager create_session B failed")))
	...
	if not bool(run_a_unwrap.get("ok", false)):
		return harness.fail_result(str(run_a_unwrap.get("error", "manager run_turn A failed")))
	...
	if not bool(snapshot_b_before_unwrap.get("ok", false)):
		return harness.fail_result(str(snapshot_b_before_unwrap.get("error", "manager get_public_snapshot B failed")))
	...
	if not bool(run_b_unwrap.get("ok", false)):
		return harness.fail_result(str(run_b_unwrap.get("error", "manager run_turn B failed")))
	...
	if not bool(snapshot_a_after_unwrap.get("ok", false)):
		return harness.fail_result(str(snapshot_a_after_unwrap.get("error", "manager get_public_snapshot A failed")))
	if not bool(snapshot_b_after_unwrap.get("ok", false)):
		return harness.fail_result(str(snapshot_b_after_unwrap.get("error", "manager get_public_snapshot B failed")))
...
func _test_session_seed_and_replay_hash_isolation(harness) -> Dictionary:
	...
	if sample_factory == null:
		return harness.fail_result("SampleBattleFactory init failed")
	var replay_input_a = harness.build_demo_replay_input(sample_factory, manager)
	...
	if not bool(replay_a1_unwrap.get("ok", false)):
		return harness.fail_result(str(replay_a1_unwrap.get("error", "run_replay A1 failed")))
	if not bool(replay_a2_unwrap.get("ok", false)):
		return harness.fail_result(str(replay_a2_unwrap.get("error", "run_replay A2 failed")))
	...
	var replay_input_b = harness.build_demo_replay_input(sample_factory, manager)
	...
	if not bool(replay_b_unwrap.get("ok", false)):
		return harness.fail_result(str(replay_b_unwrap.get("error", "run_replay B failed")))
...
func _test_replay_isolation_no_session_side_effect(harness) -> Dictionary:
	...
	if snapshot_paths_payload.has("error"):
		return harness.fail_result(str(snapshot_paths_payload.get("error", "content snapshot path build failed")))
	var snapshot_paths: PackedStringArray = snapshot_paths_payload.get("paths", PackedStringArray())
	var init_result = manager.create_session({"battle_seed": 403, "content_snapshot_paths": snapshot_paths, "battle_setup": harness.build_sample_setup(sample_factory)})
	...
	if not bool(snapshot_before_replay_unwrap.get("ok", false)):
		return harness.fail_result(str(snapshot_before_replay_unwrap.get("error", "manager get_public_snapshot failed")))
	...
	if not bool(replay_unwrap.get("ok", false)):
		return harness.fail_result(str(replay_unwrap.get("error", "run_replay failed")))
	...
	if not bool(snapshot_after_replay_unwrap.get("ok", false)):
		return harness.fail_result(str(snapshot_after_replay_unwrap.get("error", "manager get_public_snapshot after replay failed")))
	...
	var second_session = manager.create_session({"battle_seed": 404, "content_snapshot_paths": snapshot_paths, "battle_setup": harness.build_sample_setup(sample_factory)})
	...
	if not bool(count_unwrap.get("ok", false)):
		return harness.fail_result(str(count_unwrap.get("error", "manager active_session_count failed")))
