...
func test_rule_mod_refresh_updates_source_metadata_contract() -> void:
	...
	if actor == null:
		fail("missing active unit for rule_mod refresh metadata contract")
		return
	...
	if first_instance == null:
		fail("failed to create first refreshable rule_mod")
		return
	...
	if refreshed_instance == null:
		fail("failed to refresh rule_mod instance")
		return
	if refreshed_instance != first_instance:
		fail("rule_mod refresh should keep the same runtime instance")
		return
	if core.service("rule_mod_service").last_apply_skipped:
		fail("rule_mod refresh should not be marked as skipped")
		return
	if refreshed_instance.remaining != 2 or int(refreshed_instance.value) != 7:
		fail("rule_mod refresh should reset remaining turns and update value")
		return
	if refreshed_instance.source_instance_id != "refresh_source_second" or refreshed_instance.source_kind_order != 2 or refreshed_instance.source_order_speed_snapshot != 90:
		fail("rule_mod refresh should update source identity and order metadata")
		return
	if refreshed_instance.persists_on_switch != true:
		fail("rule_mod refresh should keep persists_on_switch updated on refresh")
		return
