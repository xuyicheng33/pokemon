...
func test_sukuna_default_loadout_contract() -> void:
	...
	if sukuna == null:
		fail("missing sukuna unit definition")
		return
	if sukuna.skill_ids != PackedStringArray(["sukuna_kai", "sukuna_hatsu", "sukuna_hiraku"]):
		fail("sukuna default loadout must stay fixed as 解/捌/开")
		return
	if sukuna.skill_ids.has("sukuna_reverse_ritual"):
		fail("sukuna_reverse_ritual should stay in candidate pool, not default loadout")
		return
	if sukuna.candidate_skill_ids != PackedStringArray(["sukuna_kai", "sukuna_hatsu", "sukuna_hiraku", "sukuna_reverse_ritual"]):
		fail("sukuna candidate skill pool should be structured as 解/捌/开/反转术式")
		return
	if sukuna.ultimate_skill_id != "sukuna_fukuma_mizushi":
		fail("sukuna ultimate should stay fixed as 伏魔御厨子")
		return
	if int(sukuna.ultimate_points_required) != 3 or int(sukuna.ultimate_points_cap) != 3:
		fail("sukuna ultimate point config should stay fixed as required=3 / cap=3")
		return
	if sukuna.passive_skill_id != "sukuna_teach_love":
		fail("sukuna passive should stay fixed as 教会你爱的是...")
		return
	if not content_index.skills.has("sukuna_reverse_ritual"):
		fail("sukuna candidate skill pool should retain 反转术式 for replacement tests")
		return

func test_sukuna_matchup_regen_runtime_path() -> void:
	...
	if regen_instances.size() != 1:
		fail("sukuna should receive exactly one matchup regen rule_mod on init")
		return
	...
	if int(regen_instances[0].value) != expected_value:
		fail("sukuna matchup regen rule_mod value mismatch: expected=%d actual=%d" % [expected_value, int(regen_instances[0].value)])
		return
	...
	if int(sukuna_unit.current_mp) != expected_current_mp:
		fail("sukuna initial current_mp should reflect base regen plus matchup bonus: expected=%d actual=%d" % [expected_current_mp, int(sukuna_unit.current_mp)])
		return
	...
	if int(sukuna_unit.current_mp) != expected_next_turn_mp:
		fail("sukuna next turn regen should keep base regen plus matchup bonus after the pre-applied first turn: expected=%d actual=%d" % [expected_next_turn_mp, int(sukuna_unit.current_mp)])
		return
	var regen_payload = content_index.effects["sukuna_refresh_love_regen"].payloads[0]
	if String(regen_payload.mod_op) != "add":
		fail("sukuna matchup regen payload should add the matchup table value on top of base regen")
		return
	if int(regen_payload.value) != 0:
		fail("runtime formula must not mutate shared rule_mod payload value")
		return

func test_sukuna_matchup_bst_includes_max_mp_contract() -> void:
	var core_payload = _harness.build_core()
	if core_payload.has("error"):
		fail(str(core_payload["error"]))
		return
	...
	if opponent_definition == null:
		fail("missing sample_tidekit unit definition")
		return
	...
	if sukuna_unit == null or opponent_unit == null:
		fail("missing active units for matchup bst contract")
		return
	if int(opponent_unit.max_mp) != 250:
		fail("runtime unit max_mp should mirror overridden content definition in this contract test")
		return
	...
	if regen_instance == null:
		fail("sukuna matchup regen rule_mod missing")
		return
	...
	if expected_with_max_mp == expected_without_max_mp:
		fail("max_mp contract test must cross a gap band; current fixture no longer proves the seventh-dimension assumption")
		return
	if int(regen_instance.value) != expected_with_max_mp:
		fail("sukuna matchup regen should use bst gap that includes max_mp: expected=%d actual=%d" % [expected_with_max_mp, int(regen_instance.value)])
		return
	if int(regen_instance.value) == expected_without_max_mp:
		fail("sukuna matchup regen must not fall back to six-stat bst without max_mp")
		return
