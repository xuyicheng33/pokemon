...
func test_manager_run_replay_empty_snapshot_paths_contract() -> void:
	...
	if sample_factory == null:
		...
		return
	var replay_input = _harness.build_demo_replay_input(sample_factory, manager)
	...
	if not bool(failure.get("ok", false)):
		fail(str(failure.get("error", "manager run_replay empty snapshot paths contract failed")))
		return
...
func test_manager_run_replay_invalid_input_type_contract() -> void:
	...
	if not bool(failure.get("ok", false)):
		fail(str(failure.get("error", "manager run_replay invalid input type contract failed")))
		return
...
func test_manager_run_replay_null_command_contract() -> void:
	...
	if not bool(failure.get("ok", false)):
		fail(str(failure.get("error", "manager run_replay null command contract failed")))
		return
...
func test_manager_run_replay_invalid_side_id_contract() -> void:
	...
	for test_case in cases:
		...
		if not bool(failure.get("ok", false)):
			fail(str(failure.get("error", "manager run_replay invalid side_id contract failed")))
			return
...
func test_manager_run_replay_unconsumed_future_command_contract() -> void:
	...
	if not bool(failure.get("ok", false)):
		fail(str(failure.get("error", "manager run_replay unconsumed future command contract failed")))
		return
