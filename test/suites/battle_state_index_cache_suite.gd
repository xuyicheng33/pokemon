...
func test_append_side_reflects_live_lookup_contract() -> void:
	_assert_legacy_result(_test_append_side_reflects_live_lookup_contract())

func test_direct_side_array_replacement_updates_live_lookup() -> void:
	_assert_legacy_result(_test_direct_side_array_replacement_updates_live_lookup())

func test_direct_team_unit_mutation_updates_live_lookup() -> void:
	_assert_legacy_result(_test_direct_team_unit_mutation_updates_live_lookup())

func _test_append_side_reflects_live_lookup_contract() -> Dictionary:
	...
	if battle_state.get_side("P1") != side_state:
		return {"ok": false, "error": "append_side should expose side_id immediately"}
	if battle_state.get_unit("unit_p1_a") != side_state.team_units[0]:
		return {"ok": false, "error": "append_side should expose unit_instance_id immediately"}
	if battle_state.get_unit_by_public_id("P1-A") != side_state.team_units[0]:
		return {"ok": false, "error": "append_side should expose public_id immediately"}
	return {"ok": true}

func _test_direct_side_array_replacement_updates_live_lookup() -> Dictionary:
	var battle_state = BattleStateScript.new()
	...
	if battle_state.get_side("P1") != original_side:
		return {"ok": false, "error": "precondition failed: original side should be readable"}
	...
	if battle_state.get_side("P2") != replacement_side:
		return {"ok": false, "error": "get_side should reflect sides array replacement immediately"}
	if battle_state.get_unit("unit_p2_a") != replacement_side.team_units[0]:
		return {"ok": false, "error": "get_unit should reflect sides array replacement immediately"}
	if battle_state.get_unit_by_public_id("P2-A") != replacement_side.team_units[0]:
		return {"ok": false, "error": "get_unit_by_public_id should reflect sides array replacement immediately"}
	return {"ok": true}

func _test_direct_team_unit_mutation_updates_live_lookup() -> Dictionary:
	var battle_state = BattleStateScript.new()
	var side_state = _build_side("P1", [_build_unit("unit_p1_a", "P1-A")])
	var replacement_unit = _build_unit("unit_p1_b", "P1-B")
	battle_state.append_side(side_state)
	if battle_state.get_unit("unit_p1_a") != side_state.team_units[0]:
		return {"ok": false, "error": "precondition failed: original unit should be readable"}
	...
	if battle_state.get_unit("unit_p1_b") != replacement_unit:
		return {"ok": false, "error": "get_unit should reflect direct team_units replacement immediately"}
	if battle_state.get_unit_by_public_id("P1-B") != replacement_unit:
		return {"ok": false, "error": "get_unit_by_public_id should reflect direct team_units replacement immediately"}
