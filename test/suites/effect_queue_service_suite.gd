...
func test_sort_events_cross_source_speed_tie_uses_random_tiebreak_contract() -> void:
	var service = EffectQueueServiceScript.new()
	var rng := SequenceRngService.new()
	rng.rolls = [0.8, 0.2]
	var left = _build_effect_event("event_a", "source_a")
	var right = _build_effect_event("event_b", "source_b")
