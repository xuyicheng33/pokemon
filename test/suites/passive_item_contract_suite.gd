...
func test_passive_item_sample_snapshot_contract() -> void:
	_assert_legacy_result(_test_passive_item_sample_snapshot_contract(_harness))

func test_passive_item_runtime_contract() -> void:
	_assert_legacy_result(_test_passive_item_runtime_contract(_harness))

func test_passive_item_manager_smoke_contract() -> void:
	_assert_legacy_result(_test_passive_item_manager_smoke_contract(_harness))

func test_passive_item_replay_contract() -> void:
	_assert_legacy_result(_test_passive_item_replay_contract(_harness))
...
func _test_passive_item_runtime_contract(harness) -> Dictionary:
	...
	var baseline_battle_state = harness.build_initialized_battle(
		...
		sample_factory,
		1901,
		harness.build_sample_setup(sample_factory)
	)
	...
	var item_battle_state = harness.build_initialized_battle(
		...
		sample_factory,
		1901,
		harness.build_setup_by_matchup_id(sample_factory, "passive_item_vs_sample")
	)
	core.service("battle_logger").reset()
...
func _test_passive_item_manager_smoke_contract(harness) -> Dictionary:
	...
	if sample_factory == null:
		return harness.fail_result("SampleBattleFactory init failed")
	var baseline_result = _run_manager_damage_case(harness, manager, sample_factory, harness.build_sample_setup(sample_factory), 1901)
	if baseline_result.has("error"):
		return harness.fail_result(str(baseline_result["error"]))
	var item_result = _run_manager_damage_case(harness, manager, sample_factory, harness.build_setup_by_matchup_id(sample_factory, "passive_item_vs_sample"), 1901)
...
func _test_passive_item_replay_contract(harness) -> Dictionary:
	...
	if sample_factory == null:
		return harness.fail_result("SampleBattleFactory init failed")
	var baseline_input = _build_single_strike_replay_input(harness, manager, sample_factory, harness.build_sample_setup(sample_factory), 1901)
	if baseline_input == null:
		return harness.fail_result("failed to build baseline replay input")
	var item_input = _build_single_strike_replay_input(harness, manager, sample_factory, harness.build_setup_by_matchup_id(sample_factory, "passive_item_vs_sample"), 1901)
	if item_input == null:
