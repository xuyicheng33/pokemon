...
func test_multihit_skill_execute_short_circuit_contract() -> void:
	...
	if actor == null or target == null:
		fail("missing active units for multihit execute short-circuit contract")
		return
	...
	if core.service("effect_instance_service").create_instance(on_segment_effect, target.unit_instance_id, battle_state, "test_multihit_execute_segment", 0, target.base_speed) == null:
		fail("failed to seed execute short-circuit segment listener")
		return
	...
	if damage_events.size() != 1:
		fail("execute short-circuit should emit exactly 1 damage event")
		return
	if String(damage_events[0].payload_summary).find("[execute]") == -1:
		fail("execute short-circuit should keep execute marker in public damage log")
		return
	if String(damage_events[0].payload_summary).find("segment ") != -1:
		fail("execute short-circuit should not expose multihit segment logs")
		return
	if int(target.current_mp) != before_mp:
		fail("execute short-circuit should skip on_receive_action_damage_segment triggers")
		return
