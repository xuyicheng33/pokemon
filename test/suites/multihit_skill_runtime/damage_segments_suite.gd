...
func test_multihit_skill_per_segment_mod_runtime_contract() -> void:
	...
	var battle_state = _harness.build_initialized_battle(core, content_index, sample_factory, 840, battle_setup)
	var actor = battle_state.get_side("P1").get_active_unit()
	var target = battle_state.get_side("P2").get_active_unit()
	if actor == null or target == null:
		fail("missing active units for multihit per-segment contract")
		return
	...
	if core.service("rule_mod_service").create_instance(
		...
		fail("failed to create fire-only incoming_action_final_mod")
		return
	...
	if damage_events.size() != 2:
		fail("multihit per-segment contract should emit 2 damage events")
		return
	...
	if abs(int(damage_events[0].value_changes[0].delta)) != expected_fire_damage:
		fail("segment 1 damage mismatch: expected=%d actual=%d" % [
	...
	if abs(int(damage_events[1].value_changes[0].delta)) != expected_water_damage:
		fail("segment 2 damage mismatch: expected=%d actual=%d" % [
	...
	if not is_equal_approx(float(damage_events[0].type_effectiveness), expected_fire_mul):
		fail("segment 1 should log raw fire type effectiveness")
		return
	if not is_equal_approx(float(damage_events[1].type_effectiveness), expected_water_mul):
		fail("segment 2 should log raw water type effectiveness")
		return
	if String(damage_events[0].payload_summary).find("segment 1/2") == -1 or String(damage_events[1].payload_summary).find("segment 2/2") == -1:
		fail("multihit damage logs should carry segment indices")
		return

func test_multihit_skill_stops_after_faint_contract() -> void:
	var core_payload = _harness.build_core()
	if core_payload.has("error"):
		fail(str(core_payload["error"]))
		return
	...
	var battle_setup = _harness.build_sample_setup(sample_factory)
	battle_setup.sides[1].starting_index = 2
	var battle_state = _harness.build_initialized_battle(core, content_index, sample_factory, 841, battle_setup)
	var actor = battle_state.get_side("P1").get_active_unit()
	var target = battle_state.get_side("P2").get_active_unit()
	if actor == null or target == null:
		fail("missing active units for multihit faint-stop contract")
		return
	target.current_hp = 1
	...
	if damage_events.size() != 1:
		fail("multihit should stop emitting damage after target faints")
		return
	if String(damage_events[0].payload_summary).find("segment 1/3") == -1:
		fail("faint-stop contract should only keep the first segment log")
		return

func test_multihit_skill_top_level_power_ignored_contract() -> void:
	var core_payload = _harness.build_core()
	if core_payload.has("error"):
		fail(str(core_payload["error"]))
		return
	...
	var battle_setup = _harness.build_sample_setup(sample_factory)
	battle_setup.sides[1].starting_index = 2
	var battle_state = _harness.build_initialized_battle(core, content_index, sample_factory, 842, battle_setup)
	var actor = battle_state.get_side("P1").get_active_unit()
	var target = battle_state.get_side("P2").get_active_unit()
	if actor == null or target == null:
		fail("missing active units for top-level power ignore contract")
		return
	...
	var damage_events := _collect_actor_damage_events(core.service("battle_logger").event_log, actor.public_id)
	if damage_events.size() != 1:
		fail("segmented skill should emit exactly one segment damage event in top-level power ignore probe")
		return
	...
	if actual_damage != expected_damage:
		fail("segmented skill damage should use segment power, not top-level power: expected=%d actual=%d" % [expected_damage, actual_damage])
		return
	if String(damage_events[0].payload_summary).find("segment 1/1") == -1:
		fail("segmented skill damage log should still preserve the segment 1/1 marker")
		return
