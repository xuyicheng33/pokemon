...
`tests/helpers/manual_battle_full_run.gd` 是当前 `BattleSandbox` 的唯一 headless smoke 入口；省略环境变量时，默认沿用 sandbox 基线 `gojo_vs_sample + 9101 + manual/policy`，整局通过 `BattleSandboxController.submit_action()` 推进。该脚本支持：
...
默认 `tests/run_with_gate.sh` 是 quick gate；阶段长尾回归用 `tests/run_extended_gate.sh`（先跑 quick，再跑 extended 余量）。CI 的 push / PR 默认守 quick，定时与手动 workflow 会额外跑 extended gate；完整收口用 `TEST_PROFILE=full bash tests/run_with_gate.sh`。
