...
1. 强制换下的替补由被换下方从当前合法 bench 列表中立即选 1 名；若合法列表只剩 1 名则自动锁定。
2. 若当前没有合法 bench，强制换下效果直接失效；不触发 `on_switch / on_exit`，也不改写当前 active。
...
4. 强制补位由该方从合法 bench 列表中立即选 1 名；若合法列表只剩 1 名则自动锁定。
