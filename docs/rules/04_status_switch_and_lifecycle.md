...
- `forced_replace` 规则链已冻结；当前计划在后续批次补齐到 payload 执行路径。批次落地前，运行时仍以手动换人与击倒补位为主链路。
...
1. 手动换人指令在选择阶段必须指定唯一 bench 目标 `unit_id`，队列锁定后不再改选。
...
1. 强制换下的替补选择必须走系统接口：合法 bench 候选 > 1 时返回目标 `unit_id`；若只剩 1 名则自动锁定。
...
4. 强制补位的替补选择必须走系统接口：合法 bench 候选 > 1 时返回目标 `unit_id`；若只剩 1 名则自动锁定。
