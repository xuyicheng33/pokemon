...
共享能力目录不是另一个角色注册表，它只描述“哪些共享入口允许继续复用、这些入口要求哪些共享回归，以及什么时候应该停止继续扩权”。实际消费者统一从角色 source descriptor 的 `shared_capability_ids` 派生，再导出到 committed manifest/catalog。
