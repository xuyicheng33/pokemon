...
1. 先按本模板写角色稿。
2. 再按 `docs/design/formal_character_delivery_checklist.md` 补齐资源、runtime/delivery registry、suite 和记录；正式角色 registry 的单一维护入口固定为 `config/formal_character_runtime_registry.json` + `config/formal_character_delivery_registry.json`。
...
- 若有“必须满足的跨资源不变量”，要在角色稿里写清，并在 checklist 阶段决定是否需要 `content_validator_script_path`；若需要，直接登记进 `config/formal_character_runtime_registry.json`，runtime 会统一从这份 registry 装配
...
- 若共享 suite 是正式交付面的一部分，要写明“通过 formal delivery registry 回挂”；registry 的单一维护入口仍是 `config/formal_character_delivery_registry.json`
...
- 不把“资源清单 / registry / suite / checklist”混进玩法章节里，交付动作统一交给 `formal_character_delivery_checklist.md`、`config/formal_character_runtime_registry.json` 与 `config/formal_character_delivery_registry.json`
