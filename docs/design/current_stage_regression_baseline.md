...
- extended scope（`SANDBOX_SMOKE_SCOPE=extended bash tests/check_sandbox_smoke_matrix.sh`，由 `tests/run_extended_gate.sh` 在 quick 后自动调用）：quick 之外的余量——其余 visible matchup 的 `manual/policy` + 默认 matchup 的 `policy/policy`、`manual/manual` + 其余 demo profile 的 demo replay + 全部 visible matchup 的玩家 MVP 路径
