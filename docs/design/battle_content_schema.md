...
- 正式角色的跨资源共享不变量，当前统一由 `ContentSnapshotFormalCharacterValidator` 编排；`config/formal_character_manifest.json` 是正式角色条目的唯一人工维护配置，顶层固定包含 `characters / matchups / pair_interaction_cases` 三桶，`config/formal_registry_contracts.json` 负责这几桶共享字段合同。`characters[*]` 仍承载完整角色条目，但合同固定拆成两份视图：runtime 视图只要求 `character_id / unit_definition_id / formal_setup_matchup_id / required_content_paths`，以及按需补的 `content_validator_script_path`；delivery/test 视图固定要求 `character_id / display_name / design_doc / adjustment_doc / surface_smoke_skill_id / suite_path / required_suite_paths / required_test_names / shared_capability_ids / design_needles / adjustment_needles`。运行时、测试、gate 与文档都只从 manifest domain model 派生各自视图，不再各自拼 runtime / delivery / matchup 三份事实。`matchups[*]` 目前固定要求 `p1_units / p2_units`，并允许可选 `test_only: bool` 元数据；标记为 `test_only` 的 matchup 仍可被手动构建，但不会进入 directed pair surface smoke 矩阵，同角色 mirror 这类只服务测试的 matchup 也必须显式打标。directed pair surface smoke 不再手写 catalog，而是运行时按 `matchups + characters[*].surface_smoke_skill_id` 自动生成。runtime loader 由 `src/battle_core/content/formal_validators/shared/content_snapshot_formal_character_registry.gd` 读取 manifest 角色条目，但只负责校验 runtime 视图与 validator 路径存在性；delivery/test 视图里的共享 capability suite 与 validator suite 会在派生阶段自动并入，不再要求角色条目重复回挂。真正的 validator 实例化延迟到 `ContentSnapshotFormalCharacterValidator.validate()`，并且只针对当前 snapshot 实际出现的正式角色执行。`design_needles / adjustment_needles` 固定为显式 anchor id，`required_test_names` 只保留角色私有 runtime / validator 坏例锚点，shared pair 覆盖统一交给 manifest 的 `matchups / pair_interaction_cases` + shared gate；`pair_interaction_cases[*]` 还必须显式携带 `test_name / scenario_id / matchup_id / character_ids[2] / battle_seed`，其中 `character_ids` 顺序必须匹配 `matchup_id` 的 opener 方向，且不得引用 `test_only` matchup。repo consistency gate 负责锁 manifest 完整性、资源/文档/suite 可达性、pair 覆盖、validator 坏例锚点与文档锚点。
- 共享能力目录的唯一人工维护配置固定为 `config/formal_character_capability_catalog.json`；每个 entry 都必须显式登记 `capability_id / rule_doc_paths / required_suite_paths / coverage_needles / stop_and_specialize_when`。`shared_capability_ids` 只允许引用这份 catalog 已登记的入口；共享入口的实际消费者统一从 manifest 派生，catalog 要求的共享 suite 会自动并入 delivery/test 视图，`coverage_needles` 当前表示从 `required_content_paths` 导出的语义事实 ID，不再是纯文本扫描 needle；未登记就消费、或登记后没有真实语义证据，repo consistency gate 都会直接失败。
