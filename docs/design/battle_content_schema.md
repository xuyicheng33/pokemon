...
- 正式角色的跨资源共享不变量，当前统一由 `ContentSnapshotFormalCharacterValidator` 编排；`config/formal_character_sources/` 是正式角色条目的唯一人工维护配置，其中角色 source descriptor 负责角色条目，`00_shared_registry.json` 负责共享 `matchups / capabilities`。`config/formal_character_manifest.json` 与 `config/formal_character_capability_catalog.json` 只保留为生成并提交的产物，导出的 manifest 顶层继续只保留 `characters / matchups` 两桶。`config/formal_registry_contracts.json` 继续负责 `manifest_character_runtime / manifest_character_delivery / owned_pair_interaction_spec` 三组共享字段合同。`characters[*]` 仍承载完整角色条目，但合同固定拆成两份视图：runtime 视图要求 `character_id / unit_definition_id / formal_setup_matchup_id / pair_token / baseline_script_path / pair_initiator_bench_unit_ids / pair_responder_bench_unit_ids / owned_pair_interaction_specs / required_content_paths / content_validator_script_path`；delivery/test 视图固定要求 `character_id / display_name / design_doc / adjustment_doc / surface_smoke_skill_id / suite_path / required_suite_paths / required_test_names / shared_capability_ids / design_needles / adjustment_needles`。运行时、测试、gate 与文档都只从导出的 manifest domain model 派生各自视图，不再各自拼 runtime / delivery / matchup 三份事实。`pair_token` 是 formal pair 身份字段，formal pair `matchup_id` 固定由 `{left_pair_token}_vs_{right_pair_token}` 派生；`baseline_script_path` 是 formal baseline 注册字段；`formal_setup_matchup_id` 只服务默认 formal setup 入口，不再承担 pair 身份。`matchups[*]` 目前固定要求 `p1_units / p2_units`，并允许可选 `test_only: bool` 元数据；样例、单角色 setup 与 `test_only` 特例 matchup 继续显式维护，但非 `test_only` 的 formal-vs-formal directed matchup 不再人工手写，统一由 loader 按角色清单的 `pair_token + pair_initiator_bench_unit_ids + pair_responder_bench_unit_ids` 自动派生。directed pair surface smoke 继续按派生后的 `matchups + characters[*].surface_smoke_skill_id` 自动生成。runtime loader 由 `src/battle_core/content/formal_validators/shared/content_snapshot_formal_character_registry.gd` 读取生成后的 manifest 角色条目，但只负责校验 runtime 视图与 validator 路径存在性；delivery/test 视图里的共享 capability suite 与 validator suite 会在派生阶段自动并入，不再要求角色条目重复回挂。真正的 validator 实例化延迟到 `ContentSnapshotFormalCharacterValidator.validate()`，并且只针对当前 snapshot 实际出现的正式角色执行。`design_needles / adjustment_needles` 固定为显式 anchor id，`required_test_names` 只保留角色私有 runtime / validator 坏例锚点，shared pair 覆盖统一交给 manifest 派生出的 `matchups` 与 directed interaction catalog，再由 shared gate 收口。`owned_pair_interaction_specs[*]` 是唯一手写 pair interaction 输入，固定显式携带 `other_character_id / scenario_key / owner_as_initiator_battle_seed / owner_as_responder_battle_seed`；manifest 第 0 个角色必须留空，第 `i` 个角色只能声明与 `0..i-1` 更早角色的 pair，每个无向 pair 必须且只能声明一次。manifest 角色顺序是 pair interaction ownership 的稳定输入，新角色默认追加到末尾；不得为了排序美观重排既有正式角色，除非同步迁移所有 `owned_pair_interaction_specs` 并记录决策。loader 会按 manifest 顺序为每条 owned spec 派生两条 directed interaction case，补齐 `test_name / matchup_id / scenario_key / character_ids[2] / battle_seed`，其中 `character_ids` 顺序必须匹配 `matchup_id` 的 opener 方向，且不得引用 `test_only` matchup。repo consistency gate 负责锁 manifest 完整性、资源/文档/suite 可达性、pair 覆盖、validator 坏例锚点与文档锚点，并额外锁住每条 owned spec 恰好导出两条 directed interaction case。
- 共享能力目录的唯一人工维护配置固定为 `config/formal_character_sources/00_shared_registry.json`；每个 entry 都必须显式登记 `capability_id / rule_doc_paths / required_suite_paths / required_fact_ids / stop_and_specialize_when`。`shared_capability_ids` 只允许引用这份共享 source 已登记的入口；共享入口的实际消费者统一从角色 source descriptor 派生，再导出到 committed manifest/catalog。catalog 要求的共享 suite 会自动并入 delivery/test 视图，`required_fact_ids` 当前表示从 `required_content_paths` 导出的语义事实 ID，不再是纯文本扫描 needle；未登记就消费、或登记后没有真实语义证据，repo consistency gate 都会直接失败。
