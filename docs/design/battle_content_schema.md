...
- 正式角色的跨资源共享不变量，当前统一由 `ContentSnapshotFormalCharacterValidator` 编排；`config/formal_character_manifest.json` 是唯一人工维护配置，顶层固定包含 `characters / matchups / pair_interaction_cases` 三桶。`characters[*]` 同时收口 runtime 必填面、可选 `content_validator_script_path`、设计/调整文档、suite 元数据与回归锚点；运行时、测试、gate 与文档都只从 manifest domain model 派生各自视图，不再各自拼 runtime / delivery / matchup 三份事实。directed pair surface smoke 不再手写 catalog，而是运行时按 `matchups + characters[*].surface_smoke_skill_id` 自动生成。runtime loader 由 `src/battle_core/content/formal_validators/shared/content_snapshot_formal_character_registry.gd` 读取 manifest 角色条目，但只负责校验 entry 形状与 validator 路径存在性；真正的 validator 实例化延迟到 `ContentSnapshotFormalCharacterValidator.validate()`，并且只针对当前 snapshot 实际出现的正式角色执行。`characters[*]` 的 runtime 必填字段固定为 `character_id / unit_definition_id / formal_setup_matchup_id / required_content_paths`，以及按需补的 `content_validator_script_path`；交付必填字段固定为 `display_name / design_doc / adjustment_doc / surface_smoke_skill_id / suite_path / required_suite_paths / required_test_names / design_needles / adjustment_needles`；其中 `design_needles / adjustment_needles` 固定为显式 anchor id，`required_test_names` 只保留角色私有 runtime / validator 坏例锚点，shared pair 覆盖统一交给 manifest 的 `matchups / pair_interaction_cases` + shared gate；`pair_interaction_cases[*]` 还必须显式携带 `scenario_id / matchup_id / character_ids[2] / battle_seed`。repo consistency gate 负责锁 manifest 完整性、资源/文档/suite 可达性、pair 覆盖、validator 坏例锚点与文档锚点。
