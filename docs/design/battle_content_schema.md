...
- 正式角色的跨资源共享不变量，当前统一由 `ContentSnapshotFormalCharacterValidator` 编排；`config/formal_character_manifest.json` 是正式角色条目的唯一人工维护配置，顶层固定包含 `characters / matchups / pair_interaction_specs` 三桶，`config/formal_registry_contracts.json` 负责这几桶共享字段合同。`characters[*]` 仍承载完整角色条目，但合同固定拆成两份视图：runtime 视图只要求 `character_id / unit_definition_id / formal_setup_matchup_id / pair_initiator_bench_unit_ids / pair_responder_bench_unit_ids / required_content_paths`，以及按需补的 `content_validator_script_path`；delivery/test 视图固定要求 `character_id / display_name / design_doc / adjustment_doc / surface_smoke_skill_id / suite_path / required_suite_paths / required_test_names / shared_capability_ids / design_needles / adjustment_needles`。运行时、测试、gate 与文档都只从 manifest domain model 派生各自视图，不再各自拼 runtime / delivery / matchup 三份事实。`matchups[*]` 目前固定要求 `p1_units / p2_units`，并允许可选 `test_only: bool` 元数据；样例、单角色 setup 与 `test_only` 特例 matchup 继续显式维护，但非 `test_only` 的 formal-vs-formal directed matchup 不再人工手写，统一由 loader 按角色清单和 pair bench 输入自动派生。directed pair surface smoke 继续按派生后的 `matchups + characters[*].surface_smoke_skill_id` 自动生成。runtime loader 由 `src/battle_core/content/formal_validators/shared/content_snapshot_formal_character_registry.gd` 读取 manifest 角色条目，但只负责校验 runtime 视图与 validator 路径存在性；delivery/test 视图里的共享 capability suite 与 validator suite 会在派生阶段自动并入，不再要求角色条目重复回挂。真正的 validator 实例化延迟到 `ContentSnapshotFormalCharacterValidator.validate()`，并且只针对当前 snapshot 实际出现的正式角色执行。`design_needles / adjustment_needles` 固定为显式 anchor id，`required_test_names` 只保留角色私有 runtime / validator 坏例锚点，shared pair 覆盖统一交给 manifest 派生出的 `matchups / pair_interaction_cases` + shared gate。`pair_interaction_specs[*]` 每个无向正式 pair 只写一条规格，固定显式携带 `character_ids[2] / scenario_key / forward_battle_seed / reverse_battle_seed`；loader 会派生两条 directed `pair_interaction_case`，补齐 `test_name / matchup_id / scenario_key / character_ids[2] / battle_seed`，其中 `character_ids` 顺序必须匹配 `matchup_id` 的 opener 方向，且不得引用 `test_only` matchup。repo consistency gate 负责锁 manifest 完整性、资源/文档/suite 可达性、pair 覆盖、validator 坏例锚点与文档锚点，并额外锁住每个 directed interaction case 对应的 `scenario_key`。
