...
- 边界行为：
  ...
  - `20` 点火属性固定伤害已收口到共享资源 `content/shared/effects/sukuna_shared_fire_burst_damage.tres`；`sukuna_kamado_mark`、`sukuna_kamado_explode` 与 `sukuna_domain_expire_burst` 都必须引用这同一份 payload，formal validator 会同时校验资源路径与数值形状。
- 验收点：
