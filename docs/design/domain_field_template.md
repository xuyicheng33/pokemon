...
当正式角色本身带领域时，角色稿里不要重写本文件前 1 到 7 节；只在角色稿末尾追加“领域角色差异附录”。
...
- 普通角色只用 `formal_character_design_template.md`
- 领域角色在正式角色模板基础上，再追加本附录
- 角色稿里不得重新定义公共领域对拼矩阵、`field_apply_success` 用法或双领域冲突规则
