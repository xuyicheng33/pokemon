...
|`content_snapshot_cache.gd`|按稳定 `content_snapshot_paths` 签名复用“已加载且已校验”的资源数组，再为每次 session / replay 构造 fresh `BattleContentIndex`；签名同时覆盖递归外部 `.tres/.res` 依赖、formal manifest 与内容/validator 脚本|
...
|`command_source`|`manual / resource_auto / timeout_auto`|固定 `system`|
