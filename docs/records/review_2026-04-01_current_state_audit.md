...
### 1. `stacking=none` 的重复施加仍会写出 `effect:apply_effect` 日志，和规则“重复施加无效”不完全一致
...
- 当前行为：
  ...
  - 但 `PayloadStateHandler._apply_effect_payload()` 仍会把这次返回值当成一次成功施加，继续写 `EFFECT_APPLY_EFFECT` 日志。
- 风险：
  - 运行态状态没变，但事件日志看起来像“又施加了一次”。
  - 这会让日志语义和规则口径分叉，后续如果有角色依赖“同类 none 效果重复命中”的边界，很容易把回放诊断和玩法判断搞混。
- 当前影响面：
  - 现有 Gojo / 宿傩正式内容暂时没有被这个点明显打穿，所以测试还是绿的。
  - 但这属于基础设施层的语义瑕疵，越晚修越容易扩散到新角色。

### 2. Gojo 的跨资源不变量还没进入“加载期硬校验”，目前主要靠文档 + snapshot/runtime suite 守住

- 证据：
  - `docs/records/formal_character_registry.json:2-74`
  - `docs/records/formal_character_registry.json:76-157`
...
### 3. 角色回归目前偏“内部 harness 驱动”，公开 manager 流程上的角色级端到端覆盖还不够厚
...
### 4. 角色设计文档已经开始同时承载“角色设计 + 引擎扩展方案 + 验收矩阵 + 平衡备注”，继续扩角会显著放大文档维护成本
...
- 如果你现在的目标是“尽快再接新角色”，项目能撑，但会把上面 2/3/4 这几类维护风险继续放大。
- 如果你想控制后续接角色的成本，我建议先做一轮整合规范，再扩角色。优先顺序：
  1. 修 `stacking=none` 的重复施加日志语义。
