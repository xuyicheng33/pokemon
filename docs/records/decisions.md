...
- **断裂矩阵**：(1) `start_session(dict)` ↔ `start(matchup_id, seed)`；(2) `submit_player_command(payload)` 单参 ↔ `(side_id, payload)` 双参；(3) `get_public_snapshot()` ↔ `current_snapshot()`；(4) `get_legal_action_set` ↔ `legal_actions` envelope；(5) `LOCAL_PLAYER_SIDE_ID = "0"` ↔ session 内部固定 `"P1"/"P2"`；(6) lexicon 期望 `translate_*_id` ↔ 实际 `*_display_name`；(7) streamer 期望 `set_log_text / consume_into_log_text / consume` ↔ 实际只有 `pull_increment / cursor / reset`；(8) snapshot 期望 `events / current_side_to_select / legal_actions_by_side` 三字段都不存在；(9) `LegalActionSet` 是 RefCounted 不是 Dictionary；(10) `ForcedReplaceDialog.tscn` 写了但 BattleScreen 完全没引用；(11) `WinPanel.menu_requested` 信号未连；(12) `Boot` 未读命令行 launch_config。
- **修复方向（取一头作真相）**：以 `PlayerBattleSession` 现有签名（已上线）为真相，回改 `BattleScreen.gd` 调用方；保留 lexicon `*_display_name` 命名为项目一致风格，改 BattleScreen 调用方；不重新引入反射 (`_try_new_global_class` / `ClassDB.class_exists`) — 直接 `preload(...)` + `.new()`，符合 fail-fast 与显式装配。新增 `PlayerBattleSession.current_side_to_select() / legal_action_summary(side_id)` 两个薄助手覆盖 BattleScreen 所需的 view-model 数据。
- **强制换人**：`forced_command_type` 真相是 `RESOURCE_FORCED_DEFAULT` 一个常量；玩家被 KO 后的 forced replace 不通过 `forced_command_type`，而通过 "无 skill / 无 ultimate / 无 wait / 仅 switch" 的合法命令组合识别，BattleScreen 在该状态下显式弹 `ForcedReplaceDialog`。
- **launch_config CLI**：`Boot._open()` 加 `OS.get_cmdline_user_args()` 解析，`--player_mvp` / `--sandbox` 覆盖 export 默认值；既不破坏 sandbox 默认入口（`@export var launch_config = "sandbox"` 仍是默认），也支持 `godot --headless --path . -- --player_mvp` 直接切。
- **headless smoke**：新增 `tests/helpers/player_mvp_full_run.gd`（仿 `manual_battle_full_run.gd`），用 `PlayerBattleSession + PlayerDefaultPolicy + PlayerSelectionAdapter` 双侧 policy 推进到 `battle_result.finished`，最多 64 回合，失败时 `push_error("BATTLE_PLAYER_FAILED:")`。Batch F 已用此 runner 跑通 4 个推荐 matchup（`gojo_vs_sample` 15 回合、`kashimo_vs_sample` 13 回合、`obito_vs_sample` 15 回合、`sukuna_setup` 15 回合），全部 `result_type=win / reason=elimination`。runner 接入闸门留给 Batch G。
- **顺手清理**：`test/suites/formal_character_pair_smoke/interaction_suite.gd:17/21` 同函数内 `__legacy_result` 重复声明导致 Godot 4.6.1 在 `tests/check_gdunit_gate.sh` 严格 `^WARNING:` 扫描下报错；该 warning 是 main 上的预存量。本批次顺手把第一处嵌套死代码整个删掉、第二处改名 `catalog_result`，让 quick gate 在 F 提交点真正全绿。剩余 `_assert_legacy_result` 的 8 处残留留给 Batch I 收尾。
- **不做的事**：不动 sandbox 端任何文件（adapter、view、controller、session_state、launch_config）；不重写 BattleScreen 的 view 渲染分层；不改造 `PlayerContentLexicon` 内容索引；不动 `src/battle_core/` 任何引擎代码；不为引擎错误码新增中文翻译条目。
- **决策（写入约束）**：从本次起，**新增端到端 UI 必须有 headless smoke 进 gate 才算完成**；新增 adapter / scene 与同批引入的调用方接口断裂事件不可重复发生。Batch G 强制把 `player_mvp_full_run.gd` 接进 `check_boot_smoke / check_sandbox_smoke_matrix`，并补 `player_battle_session_contract_suite` / `player_content_lexicon_contract_suite` 两个 gdUnit 契约 suite 进 quick profile。
- **验证**：`bash tests/run_with_gate.sh` 全绿；`TEST_PROFILE=extended bash tests/run_with_gate.sh` 全绿（107 quick / extended 全集 PASSED + sandbox smoke matrix PASSED）；4 推荐 matchup `player_mvp_full_run.gd` 端到端跑完。

## 2026-04-27 Batch E4：注释/命名/文档收尾

- **battle_state stable_dict 注释补全**：`src/battle_core/runtime/battle_state.gd` 第 142-147 行旧注释只列了 `Omitted from stable dict`（pending_effect_queue / _chain_context_stack / fatal_damage_records_by_target）。本批次显式补一段把 `_runtime_fault_code` / `_runtime_fault_message` 写进 stable_dict 文档段：以 `runtime_fault_code` / `runtime_fault_message` 这两个 key 出现（fields 由 `record_runtime_fault` 单写者写入，运行时 fault 一旦写入即视战斗终止，因此进 stable hash）。这样后续读者不必跳到 `to_stable_dict()` 函数体才能确认两个私有字段的去向，也避免 audit 把它们误读成"漏列在 omitted 名单里"。
- **interaction_support → interaction_shared 命名收敛**：Batch C-A 当时把 `test/suites/formal_character_pair_smoke/interaction_support.gd` 改名为 `interaction_shared.gd` 的尝试被 `tests/gates/repo_consistency_formal_character_*.py` 中三处硬编码字符串阻挡（gate 文件不在 C-A 范围）后已 revert。本批次连同 gate 一次推完：`git mv` 文件 + `.uid` 保 uid；`test/suites/formal_character_pair_smoke/interaction_suite.gd` 的 preload 路径与 `_interaction_support` → `_interaction_shared` 局部变量同步改名；`tests/gates/repo_consistency_formal_character_gate.py` 的 `PAIR_INTERACTION_SUPPORT_PATH` 常量与传参字段名一并更名为 `PAIR_INTERACTION_SHARED_PATH` / `pair_interaction_shared_path`；`tests/gates/repo_consistency_formal_character_gate_cutover.py` 的 fail message 与 local var 同步；`tests/gates/repo_consistency_formal_character_pair_capability_support.py` 中 `scan_pair_interaction_support_regressions` 函数名与 path 字面量一并改成 `scan_pair_interaction_shared_regressions` / `interaction_shared.gd`。`docs/records/decisions.md` 中 Batch C-A 章节那条「rename 已 revert」的历史记录保留不动（属于已封存时点记录）。
- **payload_handlers 二级拆分文档化**：`docs/design/effect_engine.md` §4 后面新增 §4.1 "payload_handlers 二级拆分保留情形"。明确：当前 handler 默认形态是自包含；唯一保留的二级拆分是 `payload_resource_runtime_service`，因其同时服务 heal + resource_mod 两个 handler 共享 `_apply_resource_like_change` 数值变更逻辑，是真 1:N 共享 helper（不属于 audit 当时遗留的"1:1 单 owner 隔离"形态，无法折叠）。新增 payload 的判断顺序固化在文档：(1) 数值独立 → handler 自包含；(2) 复用资源变更语义 → 走 `payload_resource_runtime_service`，不再拆第三个 runtime_service；(3) 两个以上 handler 共享非资源类执行逻辑 → 才允许新建第二个 runtime_service，并必须同步登记。明文禁止"单 handler 单 runtime_service"的二级拆分回潮（B2c 已折叠完毕）。
- **不做的事**：本批次不改 `src/adapters/player/`、`scenes/`、其它 `src/`、其它 `test/`、不改测试逻辑，仅做注释 / 命名 / 文档级别收尾；不动 `interaction_shared.gd` 文件本体内容（仅文件名变更）；不改 `tests/sync_formal_registry.sh` 或 `tests/suite_profiles.json`（无引用）。
- **验证**：`bash tests/check_architecture_constraints.sh`、`bash tests/check_repo_consistency.sh`、`bash tests/check_python_lint.sh`、`bash tests/check_suite_reachability.sh` 均 PASS。

## 2026-04-27 Batch E3：active 判定语义边界澄清（不合并）

- **目标**：核查 `turn_start_regen_service` 与 `effect_instance_dispatcher` 是否能共享同一个「什么算 active effect」查询点（B3b 已加 `EffectInstanceService.unit_has_persistent_effect / build_active_effect_public_summaries / count_matching_effect_instances` 等）。
- **调查结论（差异列表）**：
  1. `turn_start_regen_service.apply_turn_start_regen` 完全不读 `unit_state.effect_instances`。它的「active 判定」只有 `side_state.get_active_unit()`（即 primary 槽位且解析为非空 unit）+ `current_hp > 0`（活体）。回蓝数值由 `unit_state.regen_per_turn`（baseline 字段）+ `RuleModService.resolve_mp_regen_value`（读 `unit_state.rule_mod_instances`，与 `effect_instances` 是分开的两条运行态集合）共同决定。
  2. `effect_instance_dispatcher` 是 `src/battle_core/effects/` 内唯一遍历 `owner_unit.effect_instances` 的 fan-out / decrement 入口。其「active effect」判定由三层组合：（a）原始 `unit_state.effect_instances` 集合本身、（b）`effect_definition.trigger_names.has(trigger_name)`、（c）`_should_skip_reentered_persistent_turn_trigger`（持久化 effect 在 owner 本回合刚回场时跳过 `turn_start / turn_end` 触发）。
  3. `effect_instance_dispatcher._is_unit_currently_active` 是另一种语义的「active 判定」：扫描 `owner_side.active_slots.keys()` 看 owner_unit 是否落在任一槽位（多槽感知），不做活体检查；它仅闸 `on_expire` 事件 fan-out，与 regen_service 的「primary 槽位 + 活体」语义不同。
- **决策（不合并）**：两个 service 没有可收口到同一查询点的「active effect」判定。regen_service 没有 effect-instance 读点可以收口；dispatcher 与 regen_service 内部的「active unit」语义也不同（多槽感知 vs primary 槽 + 活体），合并会引入语义漂移。EffectInstanceService 的 B3b 查询 API 不在此处复用，不为 regen_service 强行制造 effect-instance 读点；`_is_unit_currently_active` 也不下沉到 EffectInstanceService（它是 active-**unit** 判定，不是 active-**effect** 判定，下沉会污染该 service 的语义边界）。
- **代码动作**：仅在 `turn_start_regen_service.gd` 与 `effect_instance_dispatcher.gd` 文件头各加一段 docstring 显式声明 active 判定边界与不合并理由，引用本条决策；不动函数体、不动 `EffectInstanceService` 现有 API、不动 `payload_handlers/` / `src/adapters/` / `scenes/` / `tests/` / `test/`。
- **验证**：`bash tests/check_architecture_constraints.sh` 通过（4 条 ARCH_GATE_PASSED）。
- **未决项**：`_is_unit_currently_active` / `get_active_unit() + alive` 这两种「active unit」判定在仓库其它若干处也存在副本（如 `leave_service / replacement_selection_helper / public_snapshot_builder / runtime_guard_service / battle_initializer_phase_service`），是否需要再统一不在 E3 范围内，留待后续批次决定。

## 2026-04-27 Batch E2：payload_service_specs 静态 preload 绑定

- **背景**：`src/composition/battle_core_payload_service_specs.gd` 旧实现按命名约定把 `handler_slot` 拼成 `res://src/battle_core/effects/payload_handlers/<slot>.gd` 后用 `ResourceLoader.exists / load(path_string)` 在运行期解析；handler 脚本一旦被重命名或删除，编译期不会任何报错，只能等到 composer 装配阶段或 architecture composition consistency gate 才暴露。本批次改为把 9 个 handler 各写成独立 `const ... := preload("res://...")`，并通过 `HANDLER_SERVICE_DESCRIPTORS` 静态表挂接到 `service_descriptors()`。
- **绑定形态**：保留 `PayloadContractRegistry.descriptors()` 作为「哪些 handler_slot 需要 service 装配」的真相输入，`service_descriptors()` 在迭代过程中按 slot 名查 `HANDLER_SERVICE_DESCRIPTORS` 派生的 dict；命中不到直接 `assert` fail-fast，与 architecture composition consistency gate 的 cross-check（`payload_handler_slots` vs `payload_handler_script_slots`）形成静态 + 运行时双层拦截。
- **保 gate 兼容**：`tests/gates/architecture_composition_consistency_gate_support.py:99-103` 的 `parse_payload_shared_service_descriptors` 正则锁定 `r"const SHARED_SERVICE_DESCRIPTORS := \[(.*?)\]\n\nstatic func service_descriptors"`；新增的 `HANDLER_SERVICE_DESCRIPTORS` 必须放在 `SHARED_SERVICE_DESCRIPTORS` **之前**，避免破坏「`SHARED_SERVICE_DESCRIPTORS` 紧邻 `service_descriptors` 函数」的紧邻约束；handler 描述符不进 SHARED 列表也避免与 gate 通过文件系统扫描派生的 handler 描述符在 `validate_service_descriptors` 触发重复 slot。
- **删除的旧形态**：`PAYLOAD_HANDLER_SCRIPT_ROOT` / `handler_script_path_for_slot()` / `_handler_service_descriptor()` / `_load_handler_script()` 这条「字符串拼路径 + 运行时 load」链路整段移除。`handler_script_path_for_slot()` 之前没有任何外部调用点（`Grep` 结果仅命中文件自身），删除安全。
- **不动的事**：`src/battle_core/effects/payload_handlers/*.gd`、`src/battle_core/content/payload_contract_registry.gd`（仍是 handler_slot → handler dependencies 的真相）、`src/composition/battle_core_payload_runtime_service_registry.gd`（runtime service 已经是 preload 形态）、所有 architecture gate 都未触动。

## 2026-04-27 Batch E1：BattleCoreSession facade 文档对齐

- **背景**：`src/battle_core/facades/battle_core_session.gd` 的 `current_battle_state()` 与 `current_content_index()` 直接返回内部 runtime 引用，被 manager 内部的 `BattleCoreManagerSessionService` 与 `BattleCorePublicSnapshotBuilder` 以及外围 manager / adapter 路径作为 production 层稳定读法使用。但 `docs/design/battle_core_architecture_constraints.md` 与 `docs/design/architecture_overview.md` 旧口径仍写「`BattleCoreSession` 只是内部会话壳，不属于外围稳定入口」，与代码实际定位前后矛盾。
- **决策**：把 `BattleCoreSession` 显式定位为 session 级稳定 facade（stable session facade），与 manager 级 `BattleCoreManager` 并列两层稳定 facade。`current_battle_state()` / `current_content_index()` / `get_legal_actions_result()` / `run_turn_result()` / `validate_runtime_result()` / `get_event_log_snapshot_result()` / `dispose()` 都属于 production 层公开 API，外围 manager / adapter 在持有 session 句柄时允许调用，包括读取 `current_battle_state()` / `current_content_index()` 返回的 runtime 引用。
- **边界**：仍禁止外围绕过 session facade 直接操作 `BattleCoreContainer` 或独立持有 runtime 引用。runtime 引用只能在 session 生命周期内被只读消费，不得缓存到脱离 session 生命周期的长寿位置；session 一旦 `dispose`，外围必须放弃其持有的 runtime 引用。`facades/` 目录里仅 `battle_core_manager.gd` + `battle_core_session.gd` 是稳定 facade，其余 `*_contract_helper / *_container_service / *_session_service` 仍是 manager 内部 helper，不升级为外围入口。
- **文档更新**：`docs/design/battle_core_architecture_constraints.md` §5 与 `docs/design/architecture_overview.md` §8 同步把「内部会话壳」措辞改成「session 级稳定 facade」，并补充 `current_battle_state / current_content_index` 的稳定 API 定位与 runtime 引用使用边界。归档目录（`docs/records/archive/*`）保留历史口径不动，因为它们是已封存的时点记录。
- **不做的事**：不动任何代码（`battle_core_session.gd` 行为本就符合稳定 facade 定位，本批次只是把文档与实际定位对齐）；不重命名类名、不改方法签名；不为 session facade 加新 envelope 出口（继续用现有 `*_result` 系列）。
