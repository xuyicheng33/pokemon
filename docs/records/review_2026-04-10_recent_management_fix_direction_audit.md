...
### 2. formal 角色交付链的治理修复已经形成单真相

- manifest 运行时视图、交付视图和 pair interaction catalog 已从单文件热点拆成 facade + helper。
- `battle_seed` 已回收到 `config/formal_registry_contracts.json` 与 `src/shared/formal_registry_contracts.gd` 的共享合同，不再只靠 loader/gate 私下补约束。
...
- `e7a336b` / `7f501bd` 把 pair contract、manifest helper、payload registry、power bonus registry 补成了真正的单点事实源。
...
- payload 已经不再多处手抄名单，但新增 payload 仍然至少会碰：
  ...
  - 对应 wiring spec
- power bonus 也已经从“名单 + 校验 + 运行时分支”三处分裂收成一处 owner，但新增 source 仍要补 registry 内部校验和解析逻辑。
