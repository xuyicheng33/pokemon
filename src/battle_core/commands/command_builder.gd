...
func build_command(input_payload: Dictionary) -> Variant:
	...
	if id_factory == null:
		ErrorStateHelperScript.fail(self, ErrorCodesScript.INVALID_STATE_CORRUPTION, "CommandBuilder missing id_factory")
		return null
...
func _fail_invalid_payload(message: String) -> Variant:
	ErrorStateHelperScript.fail(self, ErrorCodesScript.INVALID_COMMAND_PAYLOAD, message)
	return null
