...
const ReplacementChangeSetScript := preload("res://src/battle_core/lifecycle/replacement_change_set.gd")
...
func execute_replacement_lifecycle(
	...
	var rollback_change_set: ReplacementChangeSet = ReplacementChangeSetScript.collect_changes(
		battle_state,
		side_state,
		target_unit,
		selected_unit,
	...
	if leave_service.invalid_battle_code() != null:
		rollback_change_set.apply_change_set(battle_state, side_state, target_unit, selected_unit, battle_logger)
