...
var envelope: SandboxSessionCoordinatorEnvelopeHelper = EnvelopeHelperScript.new()
...
func refresh_session_snapshot_and_logs(state: SandboxSessionState, from_index: int) -> String:
	...
	state.event_log_buffer.apply_event_log_snapshot(
		...
		envelope.build_summary_context(state.launch_config, state.side_control_modes, state.command_steps)
	)
