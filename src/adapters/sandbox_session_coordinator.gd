...
func bootstrap_scene(state: SandboxSessionState, requested_config: Dictionary, policy_driver) -> Dictionary:
	...
	if not bool(close_result.get("ok", true)):
		...
		return ResultEnvelopeHelperScript.error(null, close_error_message)
	reset_state(state)
	...
	if not bootstrap_error.is_empty():
		...
		return ResultEnvelopeHelperScript.error(null, bootstrap_error)
	if state.is_demo_mode:
		...
		if not replay_error.is_empty():
			...
			return ResultEnvelopeHelperScript.error(null, replay_error)
