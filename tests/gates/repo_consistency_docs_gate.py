...
ctx.require_contains("docs/design/battle_content_schema.md", "Gojo / Sukuna / Kashimo", "schema three formal characters wording")
...
ctx.require_contains("docs/design/battle_core_architecture_constraints.md", "allowlisted SCC", "architecture runtime SCC wording")
...
ctx.require_contains("docs/design/battle_runtime_model.md", "mp_regen / incoming_accuracy / nullify_field_accuracy / incoming_action_final_mod", "runtime source stacking key scope contract")
...
ctx.require_contains("docs/records/decisions.md", "跨模块用户可见错误读取统一走 `error_state()` / `invalid_battle_code()`", "explicit error-state decision wording")
ctx.require_contains("docs/records/decisions.md", "Runtime wiring 图不是严格 DAG", "runtime SCC decision wording")
