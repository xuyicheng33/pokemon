...
- 正式角色注册表的 runtime / gate 必填字段固定为 `character_id / unit_definition_id / design_doc / adjustment_doc / suite_path / formal_setup_matchup_id / required_content_paths / required_suite_paths / required_test_names / design_needles / adjustment_needles`，以及按需补的 `content_validator_script_path`；其中 `design_needles / adjustment_needles` 现在固定写显式 anchor id，`required_test_names` 只保留角色私有 runtime / validator 锚点；`sample_setup_method` 若保留，只作为可选展示/调试元数据，不再参与 formal setup 或 gate 必填判断。
