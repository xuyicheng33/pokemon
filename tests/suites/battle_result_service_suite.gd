extends RefCounted
class_name BattleResultServiceSuite
...
func register_tests(runner, failures: Array[String], harness) -> void:
    runner.run_test("terminate_invalid_battle_reports_error", failures, Callable(self, "_test_terminate_invalid_battle_reports_error").bind(harness))
    runner.run_test("hard_terminate_invalid_state_reports_error", failures, Callable(self, "_test_hard_terminate_invalid_state_reports_error").bind(harness))
...
func _test_terminate_invalid_battle_reports_error(harness) -> Dictionary:
    ...
    if spy.reported_messages[0].find(ErrorCodesScript.INVALID_COMMAND_PAYLOAD) == -1:
...
func _test_hard_terminate_invalid_state_reports_error(harness) -> Dictionary:
    ...
    if spy.reported_messages[0].find(ErrorCodesScript.INVALID_STATE_CORRUPTION) == -1:
        return harness.fail_result("hard_terminate_invalid_state report should include invalid code")
    if spy.reported_messages[0].find("turn_loop_controller") == -1:
